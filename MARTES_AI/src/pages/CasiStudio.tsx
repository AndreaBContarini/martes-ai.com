      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {cases.map((case_) => (
            <Link
              key={case_.id}
              to={`/casi-studio/${case_.id}`}
              className="bg-black/40 rounded-xl overflow-hidden hover:transform hover:scale-105 transition-transform duration-300"
            >
              <img
                src={case_.image}
                alt={case_.title}
                className="w-full h-48 object-contain bg-white p-4 rounded-t-lg"
              />
              <div className="p-6">
                <div className="flex items-center text-sm text-gray-400 mb-4">
                  <span>
                    {format(case_.date, 'd MMMM yyyy', {
                      locale: it,
                    })}
                  </span>
                </div>
                <h2 className="text-xl font-bold mb-4">{case_.title}</h2>
                <p className="text-gray-300">{case_.description}</p>
                <div className="mt-6 p-4 bg-black/20 rounded-lg">
                  <p className="text-sm italic text-gray-300">
                    {case_.testimonial.quote}
                  </p>
                  <p className="mt-2 text-sm text-[#274f36]">
                    - {case_.testimonial.author}, {case_.testimonial.role}
                  </p>
                </div>
              </div>
            </Link>
